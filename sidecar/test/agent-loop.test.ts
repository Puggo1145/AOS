...
} from "../src/agent/context-observer";
...
let nextStream:
	| ((model: Model<Api>, signal?: AbortSignal) => AssistantMessageEventStream)
	| null = null;
...
test("happy path: ack + turnStarted + thinking + tokens + done", async () => {
	...
	expect(captured.notifications[1].params).toEqual({
		sessionId,
		turnId: "T1",
		status: "working",
	});
	...
	expect(last).toEqual({
		method: "ui.status",
		params: { sessionId, turnId: "T1", status: "done" },
	});
...
test("thinking lifecycle: thinking_delta and thinking_end are forwarded as ui.thinking before ui.token", async () => {
	...
	pushInbound({
		...
		id: 1,
		method: "agent.submit",
		params: { sessionId, turnId: "TT", prompt: "hi", citedContext: {} },
	});
	await flush(80);

	const thinking = captured.notifications.filter(
		(n) => n.method === "ui.thinking",
	);
	expect(thinking).toHaveLength(3);
	expect(thinking[0].params).toEqual({
		sessionId,
		turnId: "TT",
		kind: "delta",
		delta: "Considering ",
	});
	expect(thinking[1].params).toEqual({
		sessionId,
		turnId: "TT",
		kind: "delta",
		delta: "the request…",
	...
	const lastThinkingIdx = captured.notifications
		...
		.at(-1)!.i;
	const firstTokenIdx = captured.notifications.findIndex(
		(n) => n.method === "ui.token",
	);
...
test("thinking lifecycle: error mid-thinking synthesizes a {kind:end} before ui.error", async () => {
	...
	pushInbound({
		...
		id: 1,
		method: "agent.submit",
		params: { sessionId, turnId: "TE1", prompt: "hi", citedContext: {} },
	});
	await flush(80);

	const events = captured.notifications.map((n) => ({
		method: n.method,
		params: n.params,
	}));
	...
	const lastThinkingIdx = events
		...
		.at(-1)!.i;
	expect(events[lastThinkingIdx].params).toEqual({
		sessionId,
		turnId: "TE1",
		kind: "end",
	});
...
test("thinking lifecycle: cancel mid-thinking synthesizes a {kind:end} before ui.status done", async () => {
	...
	pushInbound({
		...
		id: 1,
		method: "agent.submit",
		params: { sessionId, turnId: "TC1", prompt: "hi", citedContext: {} },
	});
	await flush(30);
	pushInbound({
		jsonrpc: "2.0",
...
test("thinking lifecycle: end is not duplicated when provider already sent thinking_end", async () => {
	...
	pushInbound({
		...
		id: 1,
		method: "agent.submit",
		params: { sessionId, turnId: "TD1", prompt: "hi", citedContext: {} },
	});
	await flush(80);
...
test("cancel path: agent.cancel aborts the stream and emits status done", async () => {
	...
	pushInbound({
		...
		id: 1,
		method: "agent.submit",
		params: { sessionId, turnId: "T2", prompt: "hi", citedContext: {} },
	});
	await flush(30);
	pushInbound({
		jsonrpc: "2.0",
...
test("error path: typed authInvalidated reason maps to permissionDenied", async () => {
	...
	nextStream = (model) => {
		const s = new AssistantMessageEventStream();
		queueMicrotask(async () => {
			const errMsg = fakeAssistantMessage(model, {
	...
	pushInbound({
		...
		id: 1,
		method: "agent.submit",
		params: { sessionId, turnId: "T3", prompt: "hi", citedContext: {} },
	});
	await flush(60);
	...
	const statusChanges = captured.notifications.filter(
		(n) => n.method === "provider.statusChanged",
	);
	...
	const statuses = captured.notifications.filter(
		(n) => n.method === "ui.status",
	);
...
test("error path: untyped errors fall through to internalError", async () => {
	...
	nextStream = (model) => {
		...
		queueMicrotask(async () => {
			const errMsg = fakeAssistantMessage(model, {
				errorMessage: "ECONNREFUSED 8443",
			});
			s.push({ type: "error", reason: "error", error: errMsg });
			s.end();
		});
	...
	pushInbound({
		...
		id: 1,
		method: "agent.submit",
		params: { sessionId, turnId: "T4", prompt: "hi", citedContext: {} },
	});
	await flush(60);
	...
	expect(
		captured.notifications.find((n) => n.method === "provider.statusChanged"),
	).toBeUndefined();
});
...
test("conversation history: prior turn's user+assistant messages are replayed into the next request", async () => {
	...
	nextStream = (model, signal) => {
		...
		seenMessages.push(
			convo.llmMessages().map((m) => ({ role: m.role, content: m.content })),
		);
	...
	pushInbound({
		...
		id: 1,
		method: "agent.submit",
		params: { sessionId, turnId: "T1", prompt: "first", citedContext: {} },
	});
	await flush(80);
	...
	pushInbound({
		...
		id: 2,
		method: "agent.submit",
		params: { sessionId, turnId: "T2", prompt: "second", citedContext: {} },
	});
	await flush(80);

	// Turn 1 saw only its own user message.
	expect(seenMessages[0].map((m) => [m.role, m.content])).toEqual([
		["user", "first"],
	]);
...
test("agent.reset wipes conversation, aborts in-flight turn, and emits conversation.reset", async () => {
	...
	expect(
		captured.notifications.find((n) => n.method === "conversation.reset"),
	).toBeDefined();
});
...
test("multi-session: notifications carry the correct sessionId; reset of A does not affect B", async () => {
	...
	const aTokens = captured.notifications.filter(
		(n) => n.params?.sessionId === a.id && n.method === "ui.token",
	);
	const bTokens = captured.notifications.filter(
		(n) => n.params?.sessionId === b.id && n.method === "ui.token",
	);
	...
	const resetEvents = captured.notifications.filter(
		(n) => n.method === "conversation.reset",
	);
...
test("agent.cancel removes a queued steer prompt before it is inserted", async () => {
	...
	expect(captured.responses.find((r) => r.id === 3)?.result).toEqual({
		cancelled: true,
	});
...
test("provider error clears a queued steer prompt so the session can accept a new submit", async () => {
	...
	pushInbound({
		...
		id: 3,
		method: "agent.submit",
		params: { sessionId, turnId: "T3", prompt: "third", citedContext: {} },
	});
	await flush(100);
...
test("agent.submit / cancel / reset return unknownSession for an unknown sessionId", async () => {
	...
	pushInbound({
		...
		id: 2,
		method: "agent.cancel",
		params: { sessionId: "sess_nope", turnId: "T1" },
	});
	pushInbound({
		jsonrpc: "2.0",
		id: 3,
		method: "agent.reset",
		params: { sessionId: "sess_nope" },
	});
	await flush(40);
