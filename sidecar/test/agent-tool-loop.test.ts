...
function emitStream(
	events: (s: AssistantMessageEventStream) => void,
): AssistantMessageEventStream {
...
test("two-round tool-use: assistant calls a tool, then produces a final reply", async () => {
	...
	const toolNotifs = captured.notifications.filter(
		(n) => n.method === "ui.toolCall",
	);
	...
	expect(observedContexts[1].messages.map((m: any) => m.role)).toEqual([
		"user",
		"assistant",
		"toolResult",
	]);
...
test("computer-use app session is stopped after the terminal assistant reply", async () => {
	...
	(dispatcher as any).request = async (method: string, params: object) => {
		...
		if (method === RPCMethod.computerUseStopAppSession)
...
test("handler ToolUserError surfaces as isError result without aborting the turn", async () => {
	...
	scriptedRounds.push((model) => {
		return emitStream((s) => {
			const partial = fakeAssistant(
				model,
				[{ type: "text", text: "noted, retrying later" }],
				"stop",
			);
			s.push({
				type: "text_delta",
				contentIndex: 0,
				delta: "noted, retrying later",
				partial,
			});
			s.push({ type: "done", reason: "stop", message: partial });
		});
...
test("unexpected handler exception is surfaced as an isError tool result and the loop continues", async () => {
	...
	const errorEvents = captured.notifications.filter(
		(n) => n.method === "ui.error",
	);
...
test("permission denial does not execute the tool and records a non-error tool result", async () => {
	...
	scriptedRounds.push((model) =>
		emitStream((s) => {
			const partial = fakeAssistant(
				model,
				[{ type: "text", text: "permission denied noted" }],
				"stop",
			);
			s.push({
				type: "text_delta",
				contentIndex: 0,
				delta: "permission denied noted",
				partial,
			});
			s.push({ type: "done", reason: "stop", message: partial });
		}),
	...
	const approval = captured.requests.find(
		(r) => r.method === RPCMethod.permissionRequestApproval,
	);
	expect(approval).toBeDefined();
	expect(
		captured.notifications.some(
			(n) =>
				n.method === "ui.status" && n.params.status === "awaitingPermission",
		),
	).toBe(true);
	pushInbound({
		jsonrpc: "2.0",
	...
	const resultFrames = captured.notifications.filter(
		(n) =>
			n.method === "ui.toolCall" &&
			n.params.phase === "result" &&
			n.params.toolCallId === "tc_perm_write",
	);
	expect(resultFrames).toHaveLength(0);

	const toolResult = convo.messages.find(
		(m) => m.role === "toolResult" && m.toolCallId === "tc_perm_write",
	);
...
test("workspace write is allowed without approval", async () => {
	...
	toolRegistry.register(
		defineTool({
			...
			execute: async () => {
				executed = true;
				return {
					content: [{ type: "text", text: "wrote workspace file" }],
					isError: false,
				};
			},
		}),
	...
	expect(
		convo.messages.find(
			(m) => m.role === "toolResult" && m.toolCallId === "tc_workspace_write",
		),
	).toMatchObject({
		isError: false,
	});
...
test("bash asks for approval before execution", async () => {
	...
	toolRegistry.register(
		defineTool({
			...
			parameters: z
				.object({ command: z.string(), timeout: z.number().optional() })
				.strict(),
			execute: async () => {
				executed = true;
				return {
...
test("approval RPC failure is a turn-level failure instead of a model-visible tool result", async () => {
	...
	expect(executed).toBe(false);
	const toolResult = convo.messages.find(
		(m) => m.role === "toolResult" && m.toolCallId === "tc_approval_failure",
	);
...
test("permission configuration errors are turn-level failures instead of model-visible tool results", async () => {
		...
		permissionGateway: {
			async authorize() {
				throw new PermissionPolicyConfigurationError(
					'missing permission policy for tool "echo"',
				);
			},
			clearTurnGrants() {},
		},
	...
	expect(
		convo.messages.find(
			(m) =>
				m.role === "toolResult" && m.toolCallId === "tc_missing_permission",
		),
	).toBeUndefined();
	expect(captured.notifications).toContainEqual({
		method: RPCMethod.uiError,
...
test("permission policy callback errors are turn-level failures instead of model-visible tool results", async () => {
	...
	scriptedRounds.push((model) => {
		const tc: ToolCall = {
			...
			name: "write",
			arguments: {
				path: `/tmp/notch-policy-callback-${crypto.randomUUID()}.txt`,
			},
		};
		return emitStream((s) => {
			const partial = fakeAssistant(model, [tc], "toolUse");
	...
	expect(
		convo.messages.find(
			(m) =>
				m.role === "toolResult" && m.toolCallId === "tc_policy_callback_error",
		),
	).toBeUndefined();
	expect(captured.notifications).toContainEqual({
		method: RPCMethod.uiError,
		params: {
			...
			code: -32603,
			message:
				'permission policy "write" failed for tool "write": permission policy expected content to be a string',
		},
	});
});
...
test("cancelling during pending permission approval does not create a permission denial result", async () => {
	...
	scriptedRounds.push(() => {
		throw new Error(
			"loop should not continue after cancellation while waiting for permission",
		);
	});
	...
	expect(
		captured.notifications.filter(
			(n) =>
				n.method === "ui.toolCall" && n.params.phase === "permissionDenied",
		),
	).toHaveLength(0);
	const toolResult = convo.messages.find(
		(m) => m.role === "toolResult" && m.toolCallId === "tc_cancel_permission",
	);
	expect(toolResult).toMatchObject({
		role: "toolResult",
		toolName: "write",
...
test("computer-use turn grant allows subsequent same-turn actuating tools without another approval", async () => {
	...
	const approvalRequests = captured.requests.filter(
		(r) => r.method === RPCMethod.permissionRequestApproval,
	);
