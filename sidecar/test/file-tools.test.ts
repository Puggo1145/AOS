...
} from "../src/agent/tools/core/types";
...
function tempDir(): string {
	return mkdtempSync(
		join(tmpdir(), `notch-agent-tools-${Math.random().toString(36).slice(2)}-`),
	);
}
...
async function withHomeTemp<T>(
	run: (fixture: {
		root: string;
		tildePath: (path: string) => string;
	}) => Promise<T>,
): Promise<T> {
...
test("read reports when start is beyond EOF", async () => {
	...
	expect(textOf(r)).toBe(
		"(requested range starts after EOF; file has 3 lines)",
	);
...
test("read expands home paths through the shared file path policy", async () => {
	await withHomeTemp(async ({ root, tildePath }) => {
		...
		const r = await createReadTool().execute(
			{ path: tildePath("notes/home-read.txt") },
			ctx(),
		);
...
test("write overwrites an existing file and reports `Overwrote`", async () => {
	...
	const r = await createWriteTool().execute(
...
test("write resolves relative paths through the shared file path policy", async () => {
	...
	const r = await createWriteTool().execute(
		{ path: relativePath, content: "relative write" },
		ctx(),
	);
...
test("write expands home paths through the shared file path policy", async () => {
	await withHomeTemp(async ({ root, tildePath }) => {
		...
		const r = await createWriteTool().execute(
			{ path: tildePath("notes/home-write.txt"), content: "home write" },
			ctx(),
		);
...
test("update throws when old_text is missing and leaves the file untouched", async () => {
	...
	await expect(
		createUpdateTool().execute(
			{ path, old_text: "missing", new_text: "x" },
			ctx(),
		),
...
test("update expands home paths through the shared file path policy", async () => {
	await withHomeTemp(async ({ root, tildePath }) => {
		...
		const r = await createUpdateTool().execute(
			{
				path: tildePath("notes/home-update.txt"),
				old_text: "beta",
				new_text: "BETA",
			},
			ctx(),
