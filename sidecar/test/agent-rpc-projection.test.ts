...
function appendEmptyAssistantAndMarkDone(
	session: Session,
	turnId: string,
	timestamp: number,
): void {
...
test("sessionToListItem projects runtime session state to the session wire list shape", () => {
	...
	const first = session.conversation.startTurn({
		id: "t1",
		prompt: "hello",
		citedContext: {},
	});
...
test("conversationTurnToWire projects only Shell-authoritative turn fields", () => {
	...
	const turn = session.conversation.startTurn({
		...
		prompt: "hello",
		citedContext: {
			app: { bundleId: "com.example.app", name: "Example", pid: 42 },
		},
	});
...
test("conversationSnapshotToWire preserves runtime turn order for session.activate", () => {
	...
	expect(
		conversationSnapshotToWire(session.conversation).map((turn) => turn.id),
	).toEqual(["t1", "t2"]);
});
