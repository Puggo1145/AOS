...
function createGateway(
	...
	const dispatcher = {
		async request<R>(
			...
			calls.push({
				method,
				params: params as PermissionRequestApprovalParams,
				opts,
			});
...
test("ask policy sends approval request without timeout and allows on user approval", async () => {
	...
	expect(calls[0]?.params).toMatchObject({
		...
		risk: "high",
		capabilities: [
			{ capability: "filesystem.write", action: "Write file", target: path },
		],
	});
});
...
test("fullAccess permission level bypasses policy catalog checks", async () => {
	...
	const decision = await gateway.authorize(
		authInput("unknown_tool", { value: crypto.randomUUID() }),
	);
...
test("approval RPC failure fails loudly instead of becoming a tool result", async () => {
	...
	await expect(
		gateway.authorize(authInput("write", { path, content: "hello" })),
	).rejects.toThrow("Permission approval failed: approval channel broke");
});
...
test("malformed approval decision fails loudly instead of becoming user denial", async () => {
	...
	await expect(
		gateway.authorize(authInput("write", { path, content: "hello" })),
	).rejects.toThrow('Permission approval returned invalid decision "allowed"');
});
...
test("aborted approval notifies Shell to clear pending approval UI", async () => {
	...
	const input = {
		...authInput("write", {
			path: join(tmpdir(), `gateway-${crypto.randomUUID()}.txt`),
			content: "hello",
		}),
