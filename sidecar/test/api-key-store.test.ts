...
function makePair() {
	const ab = new Pipe();
	const ba = new Pipe();
	const aDispatcher = new Dispatcher(
		new StdioTransport(ba.asSource(), ab.asSink()),
		{ endpoint: "shell" },
	);
	const bDispatcher = new Dispatcher(
		new StdioTransport(ab.asSource(), ba.asSink()),
	);
...
function captureNotifications(
	...
	client.registerNotification(method, async (params) => {
		sink.push(params);
	});
}
...
test("provider.clearApiKey rejects oauth-auth providers", async () => {
	...
	let caught: unknown;
	try {
		await client.request(RPCMethod.providerClearApiKey, {
			providerId: "chatgpt-plan",
		});
	} catch (e) {
		caught = e;
...
test("provider.clearApiKey removes key and notifies; idempotent without notify", async () => {
	...
	const r1 = await client.request(RPCMethod.providerClearApiKey, {
		providerId: "deepseek",
	});
	...
	const r2 = await client.request(RPCMethod.providerClearApiKey, {
		providerId: "deepseek",
	});
...
afterEach(() => {
	if (tempHome) {
		...
		try {
			chmodSync(join(tempHome, ".notch-agent", "auth"), 0o700);
		} catch {}
		try {
...
test("provider.logout (apiKey) wipes in-memory key and notifies", async () => {
	...
	setApiKey("deepseek", "sk-existing");
	const r = await client.request(RPCMethod.providerLogout, {
		providerId: "deepseek",
	});
...
test("provider.logout (oauth) deletes token file and is idempotent on missing file", async () => {
	...
	const r1 = await client.request(RPCMethod.providerLogout, {
		providerId: "chatgpt-plan",
	});
	expect(r1).toEqual({ cleared: true });
	...
	const r2 = await client.request(RPCMethod.providerLogout, {
		providerId: "chatgpt-plan",
	});
	expect(r2).toEqual({ cleared: false });
	close();
...
test("provider.logout (oauth) fails loud on non-ENOENT filesystem errors", async () => {
	...
	let caught: unknown;
	try {
		await client.request(RPCMethod.providerLogout, {
			providerId: "chatgpt-plan",
		});
	} catch (e) {
		caught = e;
