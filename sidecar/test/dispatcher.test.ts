...
test("outbound method direction enforcement allows Bun-owned Shell requests", async () => {
	...
	const dispatcher = new Dispatcher(
		new StdioTransport(input.asSource(), output.asSink()),
	);

	const pending = dispatcher
		.request(RPCMethod.computerUseListApps, { mode: "running" })
		.catch((err) => err);
...
test("inbound ui.* request triggers MethodNotFound (low-level)", async () => {
	...
	inPipe.push(
		`${JSON.stringify({ jsonrpc: "2.0", id: "x1", method: "ui.token", params: {} })}\n`,
	);
...
test("registration validates request and notification method semantics", () => {
	...
	expect(() =>
		a.registerRequest(RPCMethod.providerStatusChanged, async () => ({})),
	).toThrow(/notification method/);
	expect(() =>
		a.registerNotification(RPCMethod.providerStatus, async () => {}),
	).toThrow(/request method/);
	expect(() => a.registerRequest("unknown.method", async () => ({}))).toThrow(
		/unknown RPC method/,
	);
	expect(() =>
		a.registerNotification("unknown.method", async () => {}),
	).toThrow(/unknown RPC method/);
