...
test("permission catalog must exactly match registered internal tools", () => {
	...
	expect(() =>
		assertRegisteredToolsMatchPermissionPolicies(registered, missingRead),
	).toThrow("missing permission policy for registered tool(s): read");

	const stalePolicy = new Map(builtinPermissionPolicyCatalog);
	stalePolicy.set("stale_tool", stalePolicy.get("read")!);
	expect(() =>
		assertRegisteredToolsMatchPermissionPolicies(registered, stalePolicy),
	).toThrow("permission policy registered for unknown tool(s): stale_tool");
});
...
test("filesystem policies allow workspace paths and ask for every other path", () => {
	...
	expect(
		evaluate("write", { path: workspaceFile, content: "hello" }),
	).toMatchObject({
		behavior: "allow",
		capabilities: [
			{
	...
	expect(
		evaluate("update", { path: workspaceFile, old_text: "a", new_text: "b" }),
	).toMatchObject({
		behavior: "allow",
		capabilities: [
			{
	...
	expect(evaluate("read", { path: outsideFile })).toMatchObject({
		behavior: "ask",
		risk: "medium",
	});
	expect(
		evaluate("write", { path: outsideFile, content: "hello" }),
	).toMatchObject({ behavior: "ask", risk: "high" });
	expect(
		evaluate("update", { path: outsideFile, old_text: "a", new_text: "b" }),
	).toMatchObject({
		behavior: "ask",
		risk: "high",
	});
...
test("filesystem policies ask when a workspace symlink escapes the workspace", () => {
	...
	const outsideRoot = join(
		tmpdir(),
		`notch-policy-outside-${crypto.randomUUID()}`,
	);
	...
	try {
		...
		expect(evaluate("read", { path: existingTarget })).toMatchObject({
			behavior: "ask",
			risk: "medium",
		});
		expect(
			evaluate("write", { path: existingTarget, content: "hello" }),
		).toMatchObject({
			behavior: "ask",
			risk: "high",
		});
		expect(
			evaluate("write", { path: newTarget, content: "hello" }),
		).toMatchObject({
			behavior: "ask",
			risk: "high",
		});
...
test("computer use policies allow read and cleanup but ask for actuating tools with a turn group", () => {
	...
	expect(
		evaluate("get_app_state", { windowId: 42, captureMode: "ax" }),
	).toMatchObject({
		behavior: "allow",
		capabilities: [{ capability: "computer.read" }],
	});
