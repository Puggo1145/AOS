...
function fakeAssistant(model: Model<Api>, text: string): AssistantMessage {
	return {
		role: "assistant",
			...
			queueMicrotask(() => {
				if (throwError) {
					...
					stream.push({
						type: "done",
						reason: "stop",
						message: fakeAssistant(model, summaryText),
					});
				}
				stream.end();
			});
...
function seedSessionWithHistory(turnsCount: number): {
	...
	for (let i = 0; i < turnsCount; i++) {
		...
		session.conversation.appendAssistant(
			id,
			fakeAssistant(makeFakeModel(100_000), `reply ${i}`),
		);
...
test("Conversation treats a manual compact preface as active-turn compaction input", () => {
	...
	session.conversation.appendAssistant(
		"T1",
		fakeAssistant(makeFakeModel(100_000), "older reply"),
	);
	...
	expect((input!.priorMessages[0] as any).content).toContain(
		"<compactionBoundary",
	);
...
test("compactConversation: result lays out [boundary, summary, ...activeSlice] and re-anchors the active turn", async () => {
	...
	if (typeof result === "symbol")
		throw new Error("expected CompactResult, got noop sentinel");
	...
	expect((session.conversation.llmMessages()[2] as any).content).toContain(
		"active",
	);
});
...
test("compactConversation: throws when there is no prior history to summarize", async () => {
	...
	const model = makeFakeModel(100_000);
	await expect(compactConversation(session, model)).rejects.toThrow(
		/no prior history/,
	);
});
