...
test("catalog has one complete semantic entry for every RPC method constant", () => {
	...
	expect(semantics.map((entry) => entry.method).sort()).toEqual(
		allMethodConstants,
	);
	for (const entry of semantics) {
		expect(entry.namespace).toBe(
			entry.method.slice(0, entry.method.indexOf(".")),
		);
		expect(entry.kind === "request" || entry.kind === "notification").toBe(
			true,
		);
		expect(
			entry.direction === "shellToBun" ||
				entry.direction === "bunToShell" ||
...
test("method-level semantics distinguish request and notification routes inside mixed namespaces", () => {
	...
	expect(
		rpcMethodSemantics(RPCMethod.permissionApprovalCancelled),
	).toMatchObject({
		direction: "bunToShell",
		kind: "notification",
	});
