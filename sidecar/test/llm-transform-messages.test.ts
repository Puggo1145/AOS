...
import {
	transformMessages,
	normalizeOpenAIResponsesToolCallId,
} from "../src/llm/providers/transform-messages";
import type {
	AssistantMessage,
	ImageContent,
	Model,
...
test("toolCall id normalization + toolResult rewrite", () => {
	...
	const normalize = (id: string) =>
		normalizeOpenAIResponsesToolCallId(id)
...
test("orphan toolCall is given a synthesized error toolResult", () => {
	...
	const a = emptyAssistant({
		content: [
			{ type: "toolCall", id: "tc1", name: "foo", arguments: {} } as ToolCall,
		],
	});
	const out = transformMessages([a], model);
...
test("assistant messages with stopReason error/aborted are dropped", () => {
	const model = makeModel();
	const errMsg = emptyAssistant({
		stopReason: "error",
		errorMessage: "boom",
		content: [{ type: "text", text: "x" } as TextContent],
	});
	const ok: UserMessage = { role: "user", timestamp: 0, content: "hi" };
	const aborted = emptyAssistant({
		stopReason: "aborted",
		content: [{ type: "text", text: "y" } as TextContent],
	});
