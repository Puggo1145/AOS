...
test("file tool path policy expands home-directory paths", () => {
	...
	expect(resolveFileToolPath("~/notes/today.md")).toBe(
		resolve(homedir(), "notes/today.md"),
	);
});

test("file tool path policy does not expand non-home tilde prefixes", () => {
	expect(resolveFileToolPath("~not-home/file.txt")).toBe(
		resolve("~not-home/file.txt"),
	);
});
...
test("file tool path policy text describes only the tilde forms it expands", () => {
	...
	expect(FILE_TOOL_PATH_PARAMETER_DESCRIPTION).not.toContain(
		"starting with `~`",
	);
});
