...
function mapStopReason(reason: string | null | undefined): {
	...
	if (reason == null || reason === "stop" || reason === "end")
...
function parseChunkUsage<TApi extends Api>(
	...
	const cacheHit = Number(
		raw.prompt_cache_hit_tokens ?? details.cached_tokens ?? 0,
	);
...
function normalizeToolCallId<TApi extends Api>(
	id: string,
	model: Model<TApi>,
): string {
	if (id.includes("|")) {
...
function convertMessages<TApi extends Api>(
	...
	const transformed = transformMessages(context.messages, model, (id) =>
		normalizeToolCallId(id, model),
	);

	if (context.systemPrompt) {
		const role =
			supportsThinking(model) && compat.supportsDeveloperRole
				? "developer"
				: "system";
	...
	for (const msg of transformed) {
		...
		const images = tr.content.filter(
			(b): b is Extract<ToolResultContent, { type: "image" }> =>
				b.type === "image",
		);
...
export function runCompletionsStream<TApi extends Api>(
	...
	const openBlock = (
		...
		blocks.push({
			kind,
			partialJson: kind === "toolCall" ? "" : undefined,
			streamIndex,
		});
		currentIdx = idx;
		if (kind === "text")
				...
				onEvent: (ev: EventSourceMessage) => {
					...
					try {
						...
						process.stderr.write(
							`[openai-completions] bad chunk JSON: ${String(err)}\n`,
						);
						return;
					}
					handleChunk(chunk);
	...
	function handleChunk(chunk: Record<string, unknown>): void {
		...
		if (typeof reasoningRaw === "string" && reasoningRaw.length > 0) {
			...
			const block = output.content[idx]! as {
				type: "thinking";
				thinking: string;
			};
			...
			for (const tcd of toolCallDeltas) {
				...
				const sameCall =
					...
					((sIdx !== undefined && meta.streamIndex === sIdx) ||
						(sIdx === undefined &&
							tcd.id !== undefined &&
							(output.content[idx!] as ToolCall).id === tcd.id));
				if (!sameCall) {
					idx = openBlock(
						"toolCall",
						() => ({
...
export const streamSimpleOpenAICompletions: SimpleStreamFunction<
	"openai-completions"
