...
import { AUTHENTICATED_SENTINEL, getEnvApiKey } from "../auth/env-api-keys";
import {
	readChatGPTToken,
	AuthInvalidatedError,
} from "../auth/oauth/chatgpt-plan";
...
function makeOutput<TApi extends "openai-responses">(
	model: Model<TApi>,
): AssistantMessage {
	return {
		role: "assistant",
...
function serializeMessage(msg: Message): Record<string, unknown>[] {
		...
		for (const block of msg.content) {
			if (block.type === "text") {
				...
				if (block.thinkingSignature) {
					items.push(
						JSON.parse(block.thinkingSignature) as Record<string, unknown>,
					);
				}
	...
	const textParts = msg.content
		.filter((b): b is import("../types").TextContent => b.type === "text")
		.map((b) => b.text);
	const images = msg.content.filter(
		(b): b is import("../types").ImageContent => b.type === "image",
	);
	const output =
		images.length > 0
			? [
					{
						type: "input_text",
						text: sanitizeSurrogates(textParts.join("\n")),
					},
					...images.map((b) => ({
						type: "input_image",
						image_url: `data:${b.mimeType};base64,${b.data}`,
...
function mapStopReason(
	...
	if (status === "requires_action" || status === "in_progress")
		return "toolUse";
...
> = (model, context, options) => {
	...
	const blocks = new Map<
		number,
		{ kind: "text" | "thinking" | "toolCall"; partialJson?: string }
	>();
		...
		try {
			...
			const parser = createParser({
				onEvent: (ev: EventSourceMessage) => {
					try {
						mapResponsesEventToAssistantEvent(
							ev,
							model,
							output,
							blocks,
...
function cleanupPartials(output: AssistantMessage): void {
	for (const block of output.content as unknown as Array<
		Record<string, unknown>
	>) {
...
function mapResponsesEventToAssistantEvent(
	...
	if (type === "response.created" || type === "response.in_progress") {
		const usage = (payload.response as Record<string, unknown> | undefined)
	...
	if (type === "response.output_item.added") {
		...
		if (itemType === "message") {
			...
			if (typeof callId !== "string" || typeof name !== "string") {
				throw new Error(
					"response.output_item.added function_call missing `call_id` or `name`",
				);
			}
			...
			throw new Error(
				`response.output_item.added unknown item.type: ${String(itemType)}`,
			);
		}
		return;
	}
	...
	const requireBlock = <K extends "text" | "thinking" | "toolCall">(
		kind: K,
	): {
		idx: number;
		meta: { kind: "text" | "thinking" | "toolCall"; partialJson?: string };
	} => {
		const itemId = payload.item_id;
		if (typeof itemId !== "string")
			throw new Error(`${type} missing string \`item_id\``);
		const idx = itemIndexToContent.get(itemId);
		if (idx === undefined)
			throw new Error(`${type} references unknown item_id: ${itemId}`);
		const meta = blocks.get(idx);
		if (!meta || meta.kind !== kind)
			throw new Error(`${type} expected block kind ${kind}, got ${meta?.kind}`);
		return { idx, meta };
	};
	...
	if (type === "response.function_call_arguments.delta") {
		...
		block.arguments = parseStreamingJson(meta.partialJson) as Record<
			string,
			unknown
		>;
		stream.push({
	...
	if (type === "response.function_call_arguments.done") {
		...
		if (typeof final !== "string")
			throw new Error(
				"response.function_call_arguments.done missing string `arguments`",
			);
	...
	if (type === "response.output_item.done") {
		...
		const itemId = item.id;
		if (typeof itemId !== "string")
			throw new Error("response.output_item.done missing `item.id`");
		const idx = itemIndexToContent.get(itemId);
		if (idx === undefined)
			throw new Error(
				`response.output_item.done references unknown item_id: ${itemId}`,
			);
	...
	if (type === "response.failed" || type === "error") {
		const errObj =
			(payload.response as Record<string, unknown> | undefined)?.error ??
			payload.error ??
			payload;
		const message =
			(errObj as Record<string, unknown>)?.message ?? JSON.stringify(errObj);
		throw new Error(
...
export const streamSimpleOpenaiResponses: SimpleStreamFunction<
	"openai-responses"
> = (model, context, simple?: SimpleStreamOptions) => {
