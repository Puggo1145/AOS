...
export function supportsThinking<TApi extends Api>(
	model: Model<TApi>,
): boolean {
...
export function supportsEffort<TApi extends Api>(
	...
	const s = spec(model);
	return s?.efforts.some((e) => e.value === value) ?? false;
}
...
export function defaultEffort<TApi extends Api>(
	model: Model<TApi>,
): string | null {
...
export function validateModelReasoning<TApi extends Api>(
	model: Model<TApi>,
): void {
	const s = spec(model);
	if (s === null) return;
	if (s.efforts.length === 0) {
		throw new Error(
			`catalog: model ${model.provider}/${model.id} declares reasoning but has no efforts`,
		);
	}
	for (const e of s.efforts) {
		if (typeof e.value !== "string" || e.value.length === 0) {
			throw new Error(
				`catalog: model ${model.provider}/${model.id} has an effort with empty value`,
			);
		}
	}
