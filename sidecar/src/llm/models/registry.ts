...
function bootstrap(): void {
	for (const [provider, models] of Object.entries(MODELS)) {
		...
		for (const [id, model] of Object.entries(
			models as Record<string, Model<Api>>,
		)) {
			inner.set(id, model);
		}
...
export function getDefaultModel<TApi extends Api = Api>(
	provider: string,
): Model<TApi> {
	const id = (DEFAULT_MODEL_PER_PROVIDER as Record<string, string | undefined>)[
		provider
	];
	if (!id)
