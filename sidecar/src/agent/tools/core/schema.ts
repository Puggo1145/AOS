...
export interface DefineToolInput<TSchema extends AnyZodType, TDetails> {
  ...
  description: string;
  parameters: TSchema;
  execute(args: z.infer<TSchema>, ctx: ToolExecContext): Promise<ToolExecResult<TDetails>>;
  applyRuntimeEffects?(
...
export function defineTool<TSchema extends AnyZodType, TDetails = unknown>(
  ...
  return {
    ...
    execute: async (args, ctx) =>
      input.execute(parseToolArguments(input.name, input.parameters, args), ctx),
...
export function validateToolArguments<TArgs>(
  ...
  return parseToolArguments(handler.spec.name, handler.parameterSchema, toolCall.arguments);
}
...
function formatIssue(issue: ZodIssue): string[] {
  if (issue.code === "unrecognized_keys") {
    ...
    return issue.keys.map((key) => `${prefix ? `${prefix}.` : ""}${key}: unknown field`);
  }
