...
const COMPUTER_USE_TOOL_DEFINITIONS: ComputerUseToolDefinition[] = [
	{
		name: "list_apps",
		description:
			"List macOS applications available to Notch Agent Computer Use.",
		parameters: z
			.object({
				mode: z
					.enum(["running", "all"])
					.describe("Use running unless installed apps are needed."),
			})
			.strict(),
	},
	{
	...
	},
	{
		...
		description:
			"Start a Computer Use app session for app state reads and mouse, keyboard, and AX element actions.",
		parameters: z
			.object({
				pid: z.number().int(),
				windowId: z.number().int(),
		...
		parameters: z
			.object({
				...
				stateId: z
					.string()
					.describe(
						"State ID from the get_app_state screenshot used for these coordinates.",
					),
...
export const COMPUTER_USE_TOOL_SPECS: ComputerUseToolSpec[] =
	COMPUTER_USE_TOOL_DEFINITIONS.map((tool) => ({
		name: tool.name,
...
export function createComputerUseTools(
	...
	return COMPUTER_USE_TOOL_DEFINITIONS.map((definition) =>
		createComputerUseRpcTool(
			definition,
			COMPUTER_USE_TOOL_BINDINGS[definition.name],
			dispatcher,
		),
	);
}
...
function createComputerUseRpcTool(
	...
	return defineTool({
		...
		execute: async (args, ctx) => {
			...
			const result = await dispatcher.request(binding.method, rpcArgs, {
				signal: ctx.signal,
			});
			return {
				content: binding.render
					? binding.render(result, ctx)
...
function renderComputerUseAppStateResult(
	result: unknown,
	ctx: ToolExecContext,
): ToolResultContent[] {
	...
	const content: ToolResultContent[] = [
		{ type: "text", text: renderComputerUseAppStateText(state) },
	];
	if (screenshot) {
...
export function renderComputerUseAppStateText(
	state: ComputerUseGetAppStateResult,
): string {
	const lines = [
		"<app_state>",
