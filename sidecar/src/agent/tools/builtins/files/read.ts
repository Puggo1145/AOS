...
const readParameterSchema = z
	.object({
		path: z.string().describe(FILE_TOOL_PATH_PARAMETER_DESCRIPTION),
		start: z
			.number()
			.describe("Optional. 1-based first line to return. Defaults to 1.")
			.optional(),
		end: z
			.number()
			.describe(
				"Optional. 1-based inclusive final line to return. Defaults to start + 499.",
			)
			.optional(),
	})
	.strict();
...
async function runRead(
	args: ReadArgs,
	ctx: ToolExecContext,
): Promise<ToolExecResult<ReadDetails>> {
	...
	if (end < start) {
		throw new ToolUserError(
			`read: invalid range ${start}-${end}; end must be >= start.`,
		);
	}
...
function normalizeLineNumber(value: number, name: string): number {
	if (!Number.isInteger(value) || value <= 0) {
		throw new ToolUserError(
			`read: invalid ${name} ${value}; must be a positive integer.`,
		);
	}
	return value;
}
