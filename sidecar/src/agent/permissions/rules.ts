...
type FilesystemPolicyInput = {
  readonly capability: Extract<CapabilityId, "filesystem.read" | "filesystem.write">;
...
export const builtinPermissionPoliciesByToolName = {
  read: (ctx) => allowWorkspacePathElseAsk(ctx, {
    capability: "filesystem.read",
    action: "Read file",
    pathArg: "path",
    ask: {
      title: "Allow file read?",
      message: "Agent wants to read a file outside the Notch Agent workspace.",
      risk: "medium",
    },
  }),

  write: (ctx) => allowWorkspacePathElseAsk(ctx, {
    ...
    ask: {
      ...
      message: "Agent wants to write a file outside the Notch Agent workspace.",
      risk: "high",
    },
  }),

  update: (ctx) => allowWorkspacePathElseAsk(ctx, {
    capability: "filesystem.write",
    action: "Update file",
    pathArg: "path",
    details({ args }): JSONValue {
    ...
    ask: {
      ...
      message: "Agent wants to update a file outside the Notch Agent workspace.",
      risk: "high",
    },
  }),
  ...
  list_apps: () => allow([{ capability: "computer.read", action: "List apps" }]),

  list_windows: () => allow([{ capability: "computer.read", action: "List windows" }]),

  get_app_state: () => allow([{ capability: "computer.read", action: "Read app state" }]),

  start_app_session: (ctx) => askComputerUse(ctx, "Start app session"),

  stop_app_session: () => allow([{ capability: "computer.cleanup", action: "Stop app session" }]),
...
function askComputerUse(ctx: PermissionPolicyContext, action: string): PermissionPolicyDecision {
  return ask({
...
function numberArgOrDefault(args: Record<string, unknown>, name: string, defaultValue: number): number {
  const value = args[name];
