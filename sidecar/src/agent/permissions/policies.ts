...
export function assertRegisteredToolsMatchPermissionPolicies(
	...
	if (missing.length > 0) {
		throw new Error(
			`missing permission policy for registered tool(s): ${missing.join(", ")}`,
		);
	}

	const stale = Array.from(policyNames)
		.filter((name) => !registered.has(name))
		.sort();
	if (stale.length > 0) {
		throw new Error(
			`permission policy registered for unknown tool(s): ${stale.join(", ")}`,
		);
	}
}
...
export function evaluatePermissionPolicy(
	...
	if (!policy) {
		throw new PermissionPolicyConfigurationError(
			`missing permission policy for tool "${ctx.toolName}"`,
		);
	}
...
export const builtinPermissionPolicyCatalog: PermissionPolicyCatalog = new Map(
	Object.entries(builtinPermissionPoliciesByToolName).map(
		([toolName, evaluate]) => {
