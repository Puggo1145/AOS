...
interface ApprovalDispatcher {
  request<R>(method: string, params: object, opts?: { signal?: AbortSignal; timeoutMs?: number }): Promise<R>;
...
export interface PermissionAuthorizer {
  authorize(input: PermissionAuthorizeInput): Promise<PermissionAuthorizeResult>;
...
export class PermissionGateway implements PermissionAuthorizer {
  ...
  async authorize(input: PermissionAuthorizeInput): Promise<PermissionAuthorizeResult> {
    ...
    if (decision.groupId && decision.groupGrantScope === "turn" && this.hasTurnGrant(input, decision.groupId)) {
      this.logDecision(input, decision, "allow");
      return { kind: "allowed" };
    }
    ...
    try {
      ...
      throw new PermissionApprovalError(`Permission approval failed: ${message}`);
    } finally {
      input.onApprovalEnd?.();
    }
    ...
    if (approvalDecision === "allow") {
      if (decision.groupId && decision.groupGrantScope === "turn") {
        this.turnGroupGrants.add(turnGrantKey(input.sessionId, input.turnId, decision.groupId));
        this.logDecision(input, decision, "allow");
      }
      return { kind: "allowed" };
    }
  ...
  private hasTurnGrant(input: PermissionAuthorizeInput, groupId: PermissionGroupId): boolean {
    return this.turnGroupGrants.has(turnGrantKey(input.sessionId, input.turnId, groupId));
  }
...
function turnGrantKey(sessionId: string, turnId: string, groupId: PermissionGroupId): string {
...
function deniedMessage(decision: Extract<PermissionPolicyDecision, { behavior: "ask" }>): string {
...
function parseApprovalDecision(result: PermissionRequestApprovalResult): "allow" | "deny" {
