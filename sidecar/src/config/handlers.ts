...
export function registerConfigHandlers(dispatcher: Dispatcher): void {
			...
			try {
				...
				if (err instanceof MalformedConfigError && err.kind !== "read") {
					...
					throw new RPCMethodError(
						RPCErrorCode.agentConfigInvalid,
						err.message,
					);
				} else {
					throw err;
	...
	dispatcher.registerRequest(
		RPCMethod.configSet,
		async (raw): Promise<ConfigSetResult> => {
			...
			if (
				...
				throw new RPCMethodError(
					...
					"config.set requires { providerId, modelId }",
				);
			}
	...
	dispatcher.registerRequest(
		...
		async (raw): Promise<ConfigSetEffortResult> => {
	...
	dispatcher.registerRequest(
		...
		async (raw): Promise<ConfigSetPermissionLevelResult> => {
	...
	dispatcher.registerRequest(
		...
		async (): Promise<ConfigMarkOnboardingCompletedResult> => {
