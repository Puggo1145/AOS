...
struct SidecarSupervisorTests {
    ...
    func unexpectedExitAfterLaunchIsFatal() {
        ...
        #expect(supervisor.state == .fatal("Sidecar exited unexpectedly with status 9. Restart Notch Agent to reconnect the agent."))
        #expect(messages == ["Sidecar exited unexpectedly with status 9. Restart Notch Agent to reconnect the agent."])
    }
