...
@testable import ComputerUseCLI
import CoreGraphics
import Foundation
import Testing

@Suite("ComputerUseCLI")
struct ComputerUseCLITests {
    ...
    func executableRejectsStandaloneCommands() async {
        let result = await ComputerUseCLIExecutable.run(arguments: ["list-apps"])

        #expect(result.exitCode == 64)
        #expect(result.stderr.contains("standalone commands were removed"))
        #expect(result.stderr.contains("ComputerUseCLI interactive"))
    }

    @Test("executable accepts short help")
    func executableAcceptsShortHelp() async {
        let result = await ComputerUseCLIExecutable.run(arguments: ["-h"])

        #expect(result.exitCode == 0)
        #expect(result.stdout.contains("ComputerUseCLI interactive"))
    ...
    @Test("get-app-type emits Notch Agent app classification for a pid")
    ...
    func leftClickAcceptsLocalCoordinate() async throws {
        ...
        await fake.setWindows([
            WindowInfo(
                id: 456,
                pid: 123,
                owner: "CoordinateTarget",
                title: "Notch Agent Button Reliability Target",
...
private actor FakeCoorTestTargetClient: CoorTestTargetClient {
    ...
    var state = CoorTestTargetState(
        pid: 777,
        windowId: 888,
        eventLogPath: "/tmp/notch-agent-coordinate-events.jsonl"
    )
