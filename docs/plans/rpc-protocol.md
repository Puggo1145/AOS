...
- `citedContext`：用户从 Notch UI 的 chip 列表中显式勾选的 OS Sense context 子集。结构：`{ app?, window?, behaviors?, appSnapshot?, clipboard? }`，仅包含用户引用的条目
- Shell 本地保留完整 `SenseContext`，**未被勾选的项永不传到 Bun**
...
| `computerUse.getAppState` | Request | `{ pid, windowTitleHint? }` | `{ stateId, axTree, screenshot }` |
| `computerUse.click` | Request | `{ pid, stateId, elementIndex }` 或 `{ pid, x, y }` | `{ success, method }` |
| `computerUse.drag` | Request | `{ pid, from: {x,y}, to: {x,y} }` | `{ success }` |
| `computerUse.typeText` | Request | `{ pid, text }` | `{ success }` |
| `computerUse.pressKey` | Request | `{ pid, key, modifiers? }` | `{ success }` |
| `computerUse.scroll` | Request | `{ pid, dx, dy }` | `{ success }` |
| `computerUse.doctor` | Request | `{}` | `{ accessibility, screenRecording, automation }` |

- `getAppState` 返回的 `stateId` 是 Kit 内部对 pid 一次 AX 树遍历结果的 handle，TTL 30s
- 使用 `elementIndex` 点击时必须带对应的 `stateId`；stateId 过期或窗口状态变化返回 `ErrStateStale`（见错误模型）
...
| `-32100` | `ErrStateStale` | `stateId` 过期或失效 |
| `-32101` | `ErrOperationFailed` | 四层降级链路全部失败 |
| `-32300 ~ -32399` | `agent.*` | Agent 层错误 |

`error.data` 承载结构化 context（如 `ErrOperationFailed` 附带四层降级链路各自状态），供 agent 判断重试或换策略。
...
| 字段 | 上限（base64 编码后） |
|---|---|
| `SenseContext.appSnapshot` | 400KB |
...
packages/
  AOSRPCSchema/                           # Swift package
    Sources/AOSRPCSchema/
      ...
      Agent.swift                         # agent.* params/results + SenseContext 引用类型
