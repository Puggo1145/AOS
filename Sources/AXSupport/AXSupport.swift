...
//    属于独立的 AXSupport Swift package，OS Sense Core 与
//    ComputerUseKit 都依赖它。"
//
// Putting these declarations in their own package is what enforces the
// "read side does not depend on write side" rule: OSSenseKit and the
// future ComputerUseKit each depend on AXSupport, and not on each
// other.
//
...
//
// Apple has not annotated Sendable on AX CF types. Both OSSenseKit and
// ComputerUseKit hand AXUIElement / AXObserver across actor boundaries
