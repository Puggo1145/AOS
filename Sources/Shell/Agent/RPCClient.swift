...
public enum RPCClientError: Error, Sendable {
    /// `rpc.hello` returned with a MAJOR version that doesn't match notchProtocolVersion.
...
public final class RPCClient: @unchecked Sendable {
    ...
    public func start() {
        ...
        let q = DispatchQueue(label: "notch-agent.rpc.reader", qos: .utility)
    ...
    private static func encodeLine<T: Encodable>(_ value: T) throws -> Data {
        ...
        // conformance harness pins. See `RPCSchema/CanonicalEncoder.swift`.
    ...
    /// loop mid-conversation. Screenshot bytes are passed by `.notch-agent/tmp/`
    ...
    private func handleInboundRequest(line: Data, method: String) async {
        if method == RPCMethod.rpcHello {
            do {
                ...
                let localMajor = Self.majorVersion(notchProtocolVersion)
                if major != localMajor {
                    let err = RPCErrorResponse(
                        id: req.id,
                        error: RPCError(
                            code: RPCErrorCode.invalidRequest,
                            message: "protocol MAJOR mismatch: remote=\(req.params.protocolVersion) local=\(notchProtocolVersion)"
                        )
                    )
                    ...
                    resolveHandshake(.failure(RPCClientError.protocolMajorMismatch(
                        ...
                        local: notchProtocolVersion
                    )))
                    return
                }
                let result = HelloResult(
                    protocolVersion: notchProtocolVersion,
                    serverInfo: ServerInfo(name: "notch-agent-shell", version: notchProtocolVersion)
                )
                let resp = RPCResponse(id: req.id, result: result)
