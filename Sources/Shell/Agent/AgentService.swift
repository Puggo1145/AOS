...
// MARK: - AgentStatus
//
// View-facing status enum mirrors `RPCSchema.TurnStatus` plus a couple of
...
public enum AgentStatus: Sendable, Equatable {
    ...
    //   - `RPCSchema.UIStatus`   — `ui.status` notification payload
    //   - `RPCSchema.TurnStatus` — per-turn state inside a `ConversationTurnWire`
        ...
        guard let sessionId = currentSessionId, let mirror = sessionStore.activeMirror else {
            ...
            sessionStore.lastActionError = SessionActionError(
                ...
                message: sessionStore.bootError
                    ?? "No active session. Restart Notch Agent or wait for session bootstrap to complete.",
                sessionId: nil
            )
            return
        }
    ...
    internal static func formatSubmitFailureMessage(error: Error) -> String {
        if let rpc = error as? RPCClientError {
            ...
            case .connectionClosed:
                return "Lost the agent connection before the message was sent. Restart Notch Agent and try again."
