...
private extension ReplyMarkdownTheme {
    ...
    var markdownTheme: Theme {
        switch self {
        case .normal: return .notchAgentPanel
        case .focused: return .notchAgentPanelFocused
        }
    }
}
...
extension Theme {
    @MainActor
    static let notchAgentPanel: Theme = Theme()
        .text {
    ...
    @MainActor
    static let notchAgentPanelFocused: Theme = Theme()
        .text {
            FontFamily(.system(.monospaced))
            FontSize(15)
            FontWeight(.semibold)
