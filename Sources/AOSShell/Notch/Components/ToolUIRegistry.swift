...
public enum ToolUIRegistry {
    ...
    private static func computerMousePresenter() -> ToolUIPresenter {
        ToolUIPresenter(
            ...
            callingBody: { args in computerMouseBody(args) },
    ...
    private static func computerKeyboardPresenter() -> ToolUIPresenter {
        ToolUIPresenter(
            ...
            callingBody: { args in computerKeyboardBody(args) },
            resultBody: { args, output, _ in computerResultWithArgs(args, output: output) },
            icon: "keyboard",
            summaryUnit: computerSummaryUnit
        )
    ...
    private static func computerAXPresenter() -> ToolUIPresenter {
        ToolUIPresenter(
            ...
            callingBody: { args in computerAXBody(args) },
            resultBody: { args, output, _ in computerResultWithArgs(args, output: output) },
            icon: "accessibility",
            summaryUnit: computerSummaryUnit
        )
...
private let computerSummaryUnit = ToolUISummaryUnit(key: "computer_use") { count in
    count == 1 ? "used computer" : "used computer \(count) times"
}

private func computerObject(_ value: JSONValue) -> [String: JSONValue]? {
    guard case let .object(obj) = value else { return nil }
    return obj
}

private func computerEvent(_ args: JSONValue) -> [String: JSONValue]? {
...
private func computerString(_ args: JSONValue, _ key: String) -> String? {
    guard let obj = computerObject(args),
...
private func computerEventString(_ args: JSONValue, _ key: String) -> String? {
...
private func computerNumber(_ args: JSONValue, _ key: String) -> String? {
    guard let obj = computerObject(args), let value = obj[key] else { return nil }
    return computerNumberString(value)
}

private func computerEventNumber(_ args: JSONValue, _ key: String) -> String? {
    guard let event = computerEvent(args), let value = event[key] else { return nil }
    return computerNumberString(value)
}

private func computerNumberString(_ value: JSONValue) -> String? {
    switch value {
    case .int(let number):
...
private func computerPoint(_ args: JSONValue, _ key: String) -> String? {
    guard let event = computerEvent(args),
...
private func computerModifiers(_ args: JSONValue) -> String? {
    guard let event = computerEvent(args),
...
private func computerKeyValueBody(_ args: JSONValue, keys: [String]) -> String? {
...
private func computerScalar(_ value: JSONValue) -> String? {
    switch value {
...
private func computerResultWithArgs(_ args: JSONValue, output: String) -> String {
    guard let body = computerActionBody(args) else { return output }
    if output.isEmpty { return body }
    return "\(body)\n\n\(output)"
}

private func computerActionBody(_ args: JSONValue) -> String? {
...
private func computerMouseLabel(_ args: JSONValue, isCalling: Bool) -> String {
    switch computerEventString(args, "kind") {
    case "click":
...
private func computerMouseBody(_ args: JSONValue) -> String? {
...
private func computerKeyboardLabel(_ args: JSONValue, isCalling: Bool) -> String {
    switch computerEventString(args, "kind") {
    case "text":
...
private func computerKeyboardBody(_ args: JSONValue) -> String? {
    let kind = computerEventString(args, "kind").map { "event: \($0)" }
...
private func computerAXLabel(_ args: JSONValue, isCalling: Bool) -> String {
...
private func computerAXActionVerb(_ action: String?, isCalling: Bool) -> String {
    switch action {
    case "press":
...
private func computerAXBody(_ args: JSONValue) -> String? {
