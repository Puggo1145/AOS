...
/// Listen-only mouse event tap used to compare Notch Agent and Codex event stamps.
    ...
    private func makeTap(location: MouseEventTapLocation, mask: CGEventMask) throws -> ActiveMouseEventTap {
        ...
        guard let tap = CGEvent.tapCreate(
            ...
            throw MouseEventObservationError(
                "failed to create \(location.rawValue) mouse event tap; grant Accessibility/Input Monitoring to the terminal running ComputerUseCLI"
            )
        }
