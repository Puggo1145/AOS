...
public struct LiveComputerUsePermissionClient: ComputerUsePermissionClient {
    ...
    private func guidanceLine(for permission: ComputerUsePermission) -> String {
        switch permission {
        case .accessibility:
            return "Grant Accessibility to the terminal app running ComputerUseCLI, then rerun the command."
        case .screenRecording:
            return "Grant Screen Recording to the terminal app running ComputerUseCLI; macOS may require restarting that terminal app."
        }
    }
}
