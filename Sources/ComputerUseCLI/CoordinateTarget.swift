...
public struct LiveCoorTestTargetClient: CoorTestTargetClient {
    ...
    private func targetExecutableURL() throws -> URL {
        if let override = ProcessInfo.processInfo.environment["NOTCH_COORDINATE_TARGET_PATH"],
           !override.isEmpty {
            let url = URL(fileURLWithPath: override)
            guard FileManager.default.isExecutableFile(atPath: url.path) else {
                throw CoorTestTargetError("NOTCH_COORDINATE_TARGET_PATH is not executable: \(url.path)")
            }
            return url
        }

        guard let cliURL = Bundle.main.executableURL else {
            throw CoorTestTargetError("cannot resolve ComputerUseCLI executable path")
        }
        let url = cliURL.deletingLastPathComponent().appendingPathComponent("CoordinateTarget")
        guard FileManager.default.isExecutableFile(atPath: url.path) else {
